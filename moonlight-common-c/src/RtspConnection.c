...
static int transactRtspMessageTcp(PRTSP_MESSAGE request, PRTSP_MESSAGE response, int expectingPayload, int *error) {
    ...
    *error = -1;
    ...
    sock = connectTcpSocket(&RemoteAddr, RemoteAddrLen, RTSP_SETUP_PORT, RTSP_TIMEOUT_SEC);
...
int performRtspHandshake() {
    ...
    sprintf(rtspTargetUrl, "rtsp%s://%s:%d", useEnet ? "ru" : "", urlAddr,
            RTSP_SETUP_PORT);
    currentSeqNumber = 1;
    hasSessionId = 0;
    ...
    if (useEnet) {
        ...
        enet_address_set_port(&address, RTSP_SETUP_PORT);
