  ...
  if(it = args.find("phrase"); it != std::end(args)) {
    if(it->second == "getservercert"sv) {
      ...
      if(config::sunshine.flags[config::flag::PIN_STDIN]) {
        ...
        // TODO: - Вот тут идет ввод пользователя
        // Так можно создать address чтобы сравнить с адресом пользователя
        // ENetAddress address;
        // enet_address_set_host(& address, user_input);
        // Надо сделать поток ввода горячих клавиш
...
void start(std::shared_ptr<safe::signal_t> shutdown_event) {
  ...
  https_server.config.port = HTTPS_PORT;
  ...
  try {
    ...
    BOOST_LOG(fatal) << "Couldn't bind http server to ports ["sv << HTTPS_PORT << ", "sv << HTTP_PORT << "]: "sv << err.what();
